# ocr_paddle_improved_extraction.py
import argparse
import json
import os

from ai_server.utils.logger import setup_logger
from knowledge_extraction.paddle_ocr_advanced import AdvancedPaddleOCRExtractor

# 创建日志记录器
logger = setup_logger('ocr_paddle_improved_extraction')


def main():
    parser = argparse.ArgumentParser(description="使用PaddleOCR增强版从PDF提取文本")
    ...
    parser.add_argument("--layout", action="store_true", help="是否使用版面分析")
    parser.add_argument("--save_layout", action="store_true", help="是否保存版面分析结果")
    ...
    # ------------------OCR识别文本------------------
    logger.info("高级PaddleOCR识别文本")
    print("-" * 100)
    print(f"启用版面分析: {'是' if args.layout else '否'}")

    # 创建OCR提取器
    ocr_extractor = AdvancedPaddleOCRExtractor(
        args.pdf,
        lang=args.ocr_lang,
    ...
    for batch_idx in range(num_batches):
        ...
        # 提取当前批次的文本
        batch_texts = ocr_extractor.extract_text_by_pages(
            batch_start,
            batch_end,
            dpi=args.dpi,
    ...
    print(f"python direct_extract_knowledge.py --json_file {all_text_file} --output {args.output} --use_gpu")
