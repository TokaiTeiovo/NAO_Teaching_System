...
║                        统一系统启动器                        ║
║                                                               ║
║  [机器人] AI服务器    - 智能对话、情感分析、LLM处理                ║
║  [网络] Web监控     - 实时监控、交互界面、数据可视化             ║
║  [书堆] 知识提取    - PDF处理、OCR识别、知识图谱生成             ║
╚═══════════════════════════════════════════════════════════════╝
"""
    print(banner)
...
class ServiceManager:
    ...
    def start_ai_server(self, host="localhost", port=8765):
        ...
        try:
            ...
            print(f"   [链接] WebSocket地址: ws://{host}:{port}")
            return True
    ...
    def start_web_monitor(self, host="127.0.0.1", port=5000, open_browser=True):
        ...
        try:
            ...
            print(f"   [成功] Web监控界面已启动 (PID: {process.pid})")
            print(f"   [全球] 访问地址: {url}")
            ...
            if open_browser:
                def open_browser_delayed():
                    ...
                    try:
                        ...
                        print(f"   [警告]  无法自动打开浏览器，请手动访问: {url}")
    ...
    def start_emotion_service(self, http_host="127.0.0.1", http_port=5001,
        ...
        try:
            ...
            print(f"   [成功] 情感识别服务已启动 (PID: {process.pid})")
            print(f"   [全球] HTTP API: {http_url}")
            print(f"   [插头] WebSocket: {ws_url}")
            return True

        except Exception as e:
            print(f"   [错误] 情感识别服务启动失败: {e}")
            return False
    ...
    def start_knowledge_extractor(self, **kwargs):
        ...
        try:
            ...
            print(f"   [成功] 知识图谱提取器已启动 (PID: {process.pid})")
            return True

        except Exception as e:
            print(f"   [错误] 知识图谱提取器启动失败: {e}")
            return False
        ...
        for service in self.processes:
            try:
                if service['process'].poll() is None:
                    ...
                    print(f"   [成功] 已停止: {service['name']} (PID: {service['process'].pid})")
                else:
                    print(f"   ℹ[符号]  服务已结束: {service['name']}")
            except Exception as e:
                print(f"   [警告]  停止服务时出错: {service['name']} - {e}")
