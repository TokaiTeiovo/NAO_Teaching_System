...
def check_requirements():
    ...
    try:
        import torch
        print("   [成功] PyTorch 已安装")
    except ImportError:
        missing_deps.append("torch")

    try:
        import transformers
        print("   [成功] Transformers 已安装")
    except ImportError:
        missing_deps.append("transformers")

    try:
        from paddleocr import PaddleOCR
        print("   [成功] PaddleOCR 已安装")
    except ImportError:
        print("   [警告]  PaddleOCR 未安装，将尝试使用EasyOCR")
        try:
            import easyocr
            print("   [成功] EasyOCR 已安装")
        except ImportError:
            missing_deps.append("paddleocr 或 easyocr")

    try:
        from pdf2image import convert_from_path
        print("   [成功] pdf2image 已安装")
    except ImportError:
...
def show_processing_plan(args):
    ...
    if args.pdf:
        print(f"   [页面] 输入源: PDF文件 ({args.pdf})")
    elif args.images:
        print(f"   [图片]  输入源: 图片文件夹 ({args.images})")
    elif args.json:
        print(f"   [备忘录] 输入源: JSON文件 ({args.json})")

    print(f"   [目标] 知识领域: {args.domain}")
    print(f"   [包裹] 批次大小: {args.batch_size}")

    if args.start_index > 0:
        print(f"   [启动] 开始索引: {args.start_index}")

    if args.max_items:
        print(f"   [符号] 最大处理数: {args.max_items}")

    print(f"   [软盘] 输出文件: {args.output}")

    if args.pdf:
        print(f"   [图片]  图像DPI: {args.dpi}")
        print(f"   [软盘] 保存图像: {'是' if args.save_images and not args.no_save_images else '否'}")

    print(f"   [放大镜左] OCR引擎: {args.ocr_engine}")
    print(f"   [机器人] 使用GPU: {'是' if args.use_gpu else '否'}")

    if args.import_neo4j:
        print(f"   [文件柜]  导入Neo4j: 是 ({args.neo4j_uri})")
    else:
        print(f"   [文件柜]  导入Neo4j: 否")
...
使用示例:
  ...
  python start_knowledge_extractor.py --pdf "large_doc.pdf" --batch-size 3 --start-index 10 --max-items 20
    ...
    print(f"\n[启动] 开始知识图谱提取...")
    print(f"   [灯泡] 提示: 处理过程中按 Ctrl+C 可以中断")
    print(f"   [备忘录] 日志文件: logs/knowledge_extractor.log")
    ...
    try:
        ...
        print(f"\n[完成] 知识图谱提取完成！")
        print(f"   [文件夹] 输出文件: shared/output/{args.output}")
        if args.import_neo4j:
            print(f"   [文件柜]  已导入Neo4j数据库")

    except KeyboardInterrupt:
        print("\n\n[符号] 收到停止信号...")
        print("[符号] 知识图谱提取已中断，中间结果已保存")
