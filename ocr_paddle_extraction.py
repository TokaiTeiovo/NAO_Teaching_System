# ocr_paddle_extraction.py
import argparse
import json
import os

from ai_server.utils.logger import setup_logger
from knowledge_extraction.paddle_ocr_extractor import PaddleOCRExtractor

# 创建日志记录器
logger = setup_logger('ocr_paddle_extraction')


def main():
    parser = argparse.ArgumentParser(description="使用PaddleOCR从PDF提取知识图谱")
    ...
    parser.add_argument("--ocr_lang", default="ch", help="OCR语言设置(ch/en/structure)")
    ...
    # ------------------OCR识别文本------------------
    logger.info("PaddleOCR识别文本")
    print("-" * 100)

    # 创建OCR提取器
    ocr_extractor = PaddleOCRExtractor(args.pdf, lang=args.ocr_lang, use_gpu=args.use_gpu)
